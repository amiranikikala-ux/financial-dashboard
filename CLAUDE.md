...
This project is indexed by GitNexus as **financial-dashboard** (5654 symbols, 15371 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
- **Breakdown proof gate** — aggregate-ის proof არ ნიშნავს decomposition-ის proof-ს. თუ მონაცემი UI-ში breakdown-ით (per-store, per-month, per-product) ცხადდება, ყოველი breakdown ცალკე უნდა შემოწმდეს წყაროსთან. „PROOFED" აკრძალულია სანამ ყველა breakdown-ი არ დაჯდა. ცრუ-ცარიელი breakdown (0 ₾) საეჭვოა — შემოწმდეს წყარო, არ ჩაითვალოს „რეალური ნული".
- **Pause-before-claim rule** — task-ს „დასრულდა"-ს მოვუნიშნავ მხოლოდ ცხადი checklist-ით: რა შემოწმდა source-თან 1:1, რა derived-ია, რა არ შემოწმდა. ღია gap-ები ცხადი ცრუ-დახურვის ნაცვლად.
- **Inherited-code audit** — bug-ის ძებნაში არ დავენდე უკვე-არსებულ helper-ებს (resolver, mapping, parser). თითოეული upstream helper-ი spot-check-ი რეალურ წყაროზე, სანამ proof დავხურავ.
