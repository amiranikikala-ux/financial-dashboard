"""Phase 2.2 — live Anthropic dog-food for `simulate_scenario`.

Goal: verify on a REAL Anthropic turn that the new what-if tool:
1. gets invoked on "if I X, what happens?" questions
2. ships `summary_ka` in its tool_result
3. the AI narrates baseline vs scenario from summary_ka instead of
   inventing numbers
4. does NOT trigger on historical lookups (→ read_data_json) or
   forecasts (→ forecast_revenue)

Runs IN-PROCESS. Cost budget: ~$0.30 total across 3 scenarios.
"""
...
def main() -> int:
    ...
    # Scenario 1: price hike with implicit elasticity.
    results.append(_run_scenario(
        agent,
        label="1. simulate_scenario — price +5% (elasticity auto)",
        message=(
            "რა მოხდება თუ ფასს ავწევ 5%-ით? მომხმარებელი ცოტას "
            "გაიკრებს — რამდენით გააფუჭებს ბოლო თვის margin-ს? "
            "უბრალოდ ცადე scenario simulation, retail-ის სტანდარტული "
            "elasticity."
        ),
        expected_tool="simulate_scenario",
        summary_ka_must_contain=["scenario", "pp"],
    ))

    # Scenario 2: wage hike (expense_change_pct).
    results.append(_run_scenario(
        agent,
        label="2. simulate_scenario — wage/expense +10%",
        message=(
            "თუ ხელფასს მთელ გუნდს 10%-ით ავუწიე, ბოლო თვის მდგომარეობაში "
            "რა დამიჯდება? net-სა და margin-ს რა ემართება?"
        ),
        expected_tool="simulate_scenario",
        summary_ka_must_contain=["scenario"],
    ))

    # Scenario 3: anti-trigger — historical lookup should route elsewhere.
    results.append(_run_scenario(
        agent,
        label="3. anti-trigger — historical margin lookup (NOT simulate)",
        message=(
            "რა margin მქონდა 2026-02-ში? უბრალოდ იყო ფაქტიური ციფრი."
        ),
        expected_tool="read_data_json",
        summary_ka_must_contain=[],
        forbid_tool="simulate_scenario",
    ))
