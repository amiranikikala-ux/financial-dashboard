...
> **განახლდა**: 2026-05-02 (per-store MegaPlus DB ingestion landed end-to-end + memory/governance hygiene cleanup — 4 commits + sync + pipeline verified, both stores ცოცხლები data.json-ში). გრძელი წაკითხვა საჭირო **არ არის**. ეს ფაილი ცოცხალი state-ია. Roadmap → `docs/MASTER_PLAN.md`. წესები → `AGENTS.md`. Evidence → `HANDOFF.md` + `HANDOFF_ARCHIVE/`.
>
> ✅ **TODAY (2026-05-02) — per-store MegaPlus + persistence fix + hygiene cleanup**:
...
> - **`a0468aa` fix(megaplus): cache rollup so data.json stays populated between ZIP refreshes** — without this, `megaplus_live` would only appear in data.json on runs that picked up a new ZIP. Now `process_newest_backup` writes `_megaplus_live.json` next to its watch folder on every successful restore; new `read_combined_rollup(folders)` reads cached JSONs every pipeline run.
> - **`c26536c` docs(handoff): close session** — first handoff write of the day (pre-hygiene).
> - **`bc11f0c` docs(hygiene): remove self-priming artifact tokens from active files** — comprehensive audit found that active files (CONTEXT_HANDOFF.md, AGENTS.md, MASTER_PLAN.md, several memory files) contained literal example tokens of the very Georgian-morphology regression artifacts they documented — circular self-priming loop. Cleanup: 7 lines in CONTEXT_HANDOFF abstracted (artifact tokens → "partial-token Georgian morphology artifacts"), 1 duplicate THIS-SESSION-CLOSED block at line 46 removed, line 309 internal Georgian errors fixed; AGENTS.md Session Pacing + Correction Escalation example tokens abstracted; MASTER_PLAN.md line 37 artifact-content fixed. Memory files updated separately (not in git): MEMORY.md, feedback_session_discipline.md, reference_oz_ground_truth_excel.md, project_megaplus_db_integration.md. 2× `*.md.backup` leftover files deleted. Verified clean via grep across all `.md` files. HANDOFF_ARCHIVE/* preserved as-is (historical record, not auto-loaded).
...
> - **C:\ ↔ origin sync RESOLVED**: C:\ branch had drifted ~10 commits behind origin (last position `6c1e24e`) and had a manual hotfix to `server.py` timeout (10→30 min, identical to landed `089e953`). Discarded duplicate working-tree change via `git checkout server.py`, then `git pull --ff-only origin main` brought C:\ to current head. Both copies aligned at `bc11f0c`.
