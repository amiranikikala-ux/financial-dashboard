...
> **განახლდა**: 2026-04-24 (**Sprint 3e LANDED** — POS terminal income per-file cache (TBC card_income + BOG POS) + **floating-point precision fix**. Totals now reproduce cleanly via Python `sum()` instead of iterative `+=` (TBC 378,734.76000001247 → 378,734.76; BOG 1,481,042.880000011 → 1,481,042.88 — same underlying rows, cleaner accumulator). Hot-cache speedup: TBC 14.9x, BOG 13.9x. 12 new regression tests.)
> **სტატუსი**: Phase 4A/4B/4C.2/4C.3 CLOSED · **Phase 2.1/2.2/2.4/2.5/2.6/2.8/2.9 COMPLETE** · Tier 1 + Tier 2 Sprint 1/2/3a/**3b/3c/3d/3e** COMPLETE · **🧾 Sprint 5.1-5.6, 5.8-5.11 COMPLETE · 5.7 DROPPED (retail-only) · 5.12 evidence-only** · Phase 4C.1 VAT-tools-scoped ✅ · **Sprint 3e POS income cache 22c5ac5**.
...
| მეტრიკა | მნიშვნელობა |
|---|---|
| **pytest** | **2,107/2,107 green** (~122s full run; +34 regression pins Sprint 3b/3c/3d/3e incremental cache) |
...
- Sprint 3e (`22c5ac5`) — POS terminal income per-file cache + precision fix; TBC 14.9x / BOG 13.9x hot speedup; 12 new tests (2,107/2,107 green); user-approved behavior change — totals via `sum()` not iterative `+=` (removes ~1e-8 fp drift)
