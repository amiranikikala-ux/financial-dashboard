...
> **განახლდა**: 2026-04-24 (**Sprint 3d LANDED** — cross-bank tax_flow per-file cache. Live-verify on real corpus: 31.13s → 0.055s (**561x speedup**), digest byte-identical across baseline/cold/hot. Cumulative per-file cache now covers 5 xlsx corpora (retail_sales + samurneo + expense_categories + tax_flow). 8 new regression tests.)
> **სტატუსი**: Phase 4A/4B/4C.2/4C.3 CLOSED · **Phase 2.1/2.2/2.4/2.5/2.6/2.8/2.9 COMPLETE** · Tier 1 + Tier 2 Sprint 1/2/3a/**3b/3c/3d** COMPLETE · **🧾 Sprint 5.1-5.6, 5.8-5.11 COMPLETE · 5.7 DROPPED (retail-only) · 5.12 evidence-only** · Phase 4C.1 VAT-tools-scoped ✅ · **Sprint 3d tax_flow cache (pending commit)**.
...
| მეტრიკა | მნიშვნელობა |
|---|---|
| **pytest** | **2,095/2,095 green** (~77s full run; +22 regression pins Sprint 3b/3c/3d incremental cache) |
...
- Sprint 3d (_pending commit_) — cross-bank tax_flow per-file cache landed, 561x hot speedup on real corpus, 8 new tests (2,095/2,095 green); single combined `_run_cached_per_file` call with path-dispatching processor (BOG/TBC paths disjoint)
