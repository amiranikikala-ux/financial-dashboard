...
>
> 🧹 **`SupplierModal.jsx` comment cleanup** — line 807 had a garbage-Georgian sentence (incoherent partial-token chain) inside a code comment block. Rewrote as clean Georgian. Memory rule `feedback_pause_before_claim.md` line 16 had the same broken-locative-form issue → rewritten without the broken token. **Both edits committed alongside handoff doc**.
>
> ⚠️ **Language regression caught mid-session** — same broken-Georgian-filler pattern documented in `feedback_no_garbage_georgian_tokens.md` reappeared in a single mid-session response (Lactalis explanation), user pinged it directly ("გაურკვეველი სიტყვები რომელსაც იმეორებ"). Cleaned source-file instances, but the real cause was MY OWN output regression, not the source files. Per AGENTS.md Correction Escalation: this session had 3+ Georgian filler regressions → recommended user start fresh session for next work block, user opted for break instead. **For next session**: known failure mode — mid-session output drift on the documented broken-token; mitigate via shorter responses + avoiding mixed-language paragraphs. (Concrete tokens deliberately omitted from this handoff to avoid self-priming.)
>
> 📌 **OPEN — for next session** (priority order):
>
...
>   2. ნიკა ლუკა 69 (`0951251552`) and ფუდმარტი (`0915640584`) — user said "in MegaPlus", my SQL said "not in GET" — never reconciled the discrepancy or asked WHERE in MegaPlus user was looking
>   3. ჯიდიაი `0932869245/25` — both stores have identical 1203.75 entries (real duplication signal); my UI showed sum 2407.50 which user couldn't see anywhere → looked false
>   4. MegaPlus-only rows showed `(უცნობი)` supplier — I forgot to JOIN AGREEMENTS to resolve supplier name
> - **User's exact words (paraphrased)**: "I trusted you. These numbers can hurt me because I rely on this project. Don't make excuses, make a perfect project. You're a professional — don't disappoint me. Remember my words."
> - **Action taken**: reverted EVERYTHING (`git checkout` on api_contracts.py / megaplus_backup.py / generate_dashboard_data.py / App.jsx / tabConfig.js; deleted dashboard_pipeline/waybill_reconciliation.py + WaybillReconciliation.jsx + tests/test_waybill_reconciliation.py). No commits landed for this feature. Frontend rebuilt + dist synced. Pipeline triggered (was running at handoff time) — data.json's stale `waybill_reconciliation` key will clear on next pipeline run since calling code is gone.
> - **Findings doc landed**: `f0de03a docs(handoff): waybill_reconciliation REVERTED + 4 false-positive findings` → `HANDOFF_ARCHIVE/PREVIEWS/WAYBILL_RECON_FINDINGS_2026-05-02.md` — documents each case, what's actually in MegaPlus DB vs my classification, and the protocol to follow on rebuild.
> - **Memory rule added**: `feedback_no_detector_without_breakdown_proof.md` — for ANY detection tool, random-sample 10 rows verified in source BEFORE shipping. Aggregate counts dropping after a fix do NOT prove other false-positive classes are absent.
>
> 🟢 **STILL VALID (built earlier this same date, in active use)**: `category_anomalies` tab — 1,976 empty + 695 duplicate-cluster products surfaced; user is actively cleaning MegaPlus by drop-fix-rerun cycle. NOT touched by the revert.
>
> ✅ **TODAY (2026-05-02 post-midnight) — Phase 2: category-anomaly detection table SHIPPED end-to-end + post-ship 400 fix**:
>
> ✅ **TODAY (2026-05-02 post-midnight) — Phase 2: category-anomaly detection table SHIPPED end-to-end + post-ship 400 fix**:
>
> **`2e16485` fix(api-contracts): register category_anomalies tab in allowlist + static response** — feat e128669 wrote the data into `data.json` and the frontend tab, but missed the four registrations in `dashboard_pipeline/api_contracts.py` that the API server uses to gate `/api/data?tab=...` responses. Symptom: tab loaded → frontend fetched `/api/data?tab=category_anomalies` → server returned HTTP 400 "Invalid tab" → empty state rendered. Four entries added: `FIELD_DEFAULTS`, `TAB_ALLOWLIST`, `TAB_RESPONSE_META`, `STATIC_RESPONSE_TABS`. Required `Restart-Service FinancialDashboardBackend` (admin/UAC) to load new ALLOWED_TABS into the running uvicorn process — user did this manually since I lack the service-control privilege. **Lesson for next feature ship**: any new tab needs BOTH `data["new_tab"]` populated AND four lines in api_contracts.py BEFORE the feat commit; verify with `curl /api/data?tab=new_tab` before declaring shipped. The earlier in-process dogfood scripts didn't catch this because they bypassed the API layer.
>
> ✅ **`e128669` feat(category-anomalies): per-store MegaPlus anomaly detection + dashboard tab** — three operator-error detectors run inside `_read_supplier_rollups` against each per-store MEGAPLUS_<storeID> DB and persist into `_megaplus_live.json` next to the watch folder, then `generate_dashboard_data.py` aggregates both stores into `data["category_anomalies"] = {totals, stores: [...]}`.
>   - **Detector 1 — empty category**: `WHERE P_ACTIVE=1 AND (P_GROUP IS NULL OR LTRIM(RTRIM(P_GROUP))='')`. დვაბზუ 788, ოზურგეთი 1188, total 1976 actionable rows. Each carries product_id / code / barcode (13-digit) / name / supplier so the operator finds the product fast in MegaPlus.
>   - **Detector 2 — duplicate variants**: normalize raw P_GROUP by stripping `^[\d.]+\s*\|\s*` prefix + collapsing whitespace + lowercase. Clusters with >1 raw variant emit minority-variant products as fix candidates. დვაბზუ 44 clusters / 397 minority products, ოზურგეთი 39 / 298. Bonus catch: outright miscategorizations where minority variant happens to normalize to the cluster root (e-cigarettes filed under "07201 | სანთებელა" surfaced this way).
>   - **Detector 3 — PROTECTED-supplier overview**: read-only roll-up per (supplier × category) for ELIZI 204920381 / ჯიდიაი 406181616 / ინტერნეიშნლ 420424393. Pre-emptive — if a future product attributed to one of these tax_ids drifts into an unexpected category, it surfaces here at a glance. Today's snapshot: ELIZI 100% სიგარეტი (clean); ჯიდიაი 5 categories (სიგარეტი 96-97 / გასახურებელი თამბაქო=IQOS 60 / მოსაწევი მოწყობილობა 14 / ელ. სიგარეტი 2-3 / აქსესუარები 1 — all expected); ინტერნეიშნლ 92% სიგარეტი + 2 in აქსესუარები.
>   - **Frontend**: new `🚨 კატეგორიები` tab in the sales group (`tabConfig.js`). `rs-dashboard/src/CategoryAnomalies.jsx` (~250 lines, lazy-loaded) renders one filterable table — store filter (ყველა / დვაბზუ / ოზურგეთი), problem-type filter (ყველა / ცარიელი / დუბლიკატი) — plus side-by-side PROTECTED supplier overview cards. Columns match the operator's MegaPlus search workflow: store / product name / 13-digit barcode / problem badge / current category / suggested target category. Built + dist synced to C:\.
>   - **Tests**: 24 new in `tests/test_category_anomalies.py` (normalize parametrize, cluster grouping with mock cursor, PROTECTED constant drift detector, end-to-end build_anomaly_bundle on a scripted MockCursor). 79/79 green when combined with supplier_profitability + retail_revenue.
>   - **Cache backfill**: existing `_megaplus_live.json` (one per watch folder) patched in-place via a one-time scratch script — avoided a 3-min/store SQL re-restore. Next pipeline run loads new key from cache; subsequent ZIP arrivals will recompute via the wired path.
>   - **Pipeline log proof**: `2026-05-02 01:34:24 | INFO | __main__ | category_anomalies → data.json: ცარიელი=1976, დუბლიკატი ჯგუფები=83 (პროდუქტები=695), PROTECTED კატეგორიები=16`.
>   - **Estimated 2-3 sessions in prior handoff** — closed in 1 session because (a) DB schema discovery via PRODUCTS column dump went smoothly (P_ACTIVE not P_ACT, P_GROUP for category, P_DAFAULTSUPPLIER → DISTRIBUTORS for tax_id), (b) duplicate normalization regex caught 100% of clusters on first try (44+39 from 1640 raw categories), (c) frontend reuses StoreCompare's inline-style pattern + lazy-loaded chunk pattern.
>
> 🧹 **Workflow user adopted for cleanup** (his own words paraphrased): "I'll clean gradually — fix one batch in MegaPlus, drop tomorrow's PLUS_*.zip backup, pipeline picks up the fixes, those rows disappear from the table. Then next batch the day after." Filters (store + problem type) let him slice the 2,671 rows into manageable chunks (e.g. 788 დვაბზუ + ცარიელი only). Tab UX is not the bottleneck — operator's own MegaPlus editing time is. **Do NOT ship UI follow-ups (deep-link, severity scoring, Excel export) until user explicitly asks.**
>
> 📌 **OPEN — for next session** (priority order):
> 1. **🔴 waybill_reconciliation rebuild — IF user asks**. Findings doc `HANDOFF_ARCHIVE/PREVIEWS/WAYBILL_RECON_FINDINGS_2026-05-02.md` has every case dissected + verification protocol. **DO NOT just rebuild** — first ask user where in MegaPlus they saw ნიკა ლუკა / ფუდმარტი (cases 1 & 2); without that diagnostic info we can't be sure my SQL was wrong vs user looked at a different module. Once user clarifies, build with strict random-sample-10 protocol per `feedback_no_detector_without_breakdown_proof.md`. **DEFAULT: do not bring this up unless user does** — they may just want to leave it shelved.
...
> 3. **🟢 Auto-sync OneDrive↔C:\** — drift bit me again this session (running pipeline at session start was using old code, forced manual /api/refresh after sync). post-commit/post-merge hook or scheduled task. Light task.
> 4. **🟢 `retail_sales_top_products` SQLite export bug** (carryover) — single-line fix in `dashboard_pipeline/export_sqlite.py:111` reads `(retail_sales).top_products` but schema has `top_products_by_revenue` and `top_products_by_profit`. SQLite table has been 0 rows since forever.
> 5. **🟢 ოზურგეთი DB 234 orders dated 2009** (carryover) — 0.026% of orders, 661 ₾. Likely legacy seed/test rows. Optional: filter `WHERE ORD_TIMESTAMP >= '2023-01-01'`.
> 6. **🟢 Category-anomalies — possible UI follow-ups (DO NOT ship unless user asks)**: (a) "MegaPlus deep-link" column — open a row directly in MegaPlus admin (would need URL pattern from user; user fixes via direct MegaPlus search by barcode now), (b) severity scoring — sort by financial impact (qty × price), (c) export to Excel for offline batch fix.
> 7. **🟢 rs.ge automation deferred** — Playwright-based partial automation discussed; user opted for status-quo manual download. Revisit only if signal of pain.
>
> 📜 **History — earlier this same date (2026-05-02 ღამე) — PROTECTED rule reframed: tax-id-driven, not category-driven**:
...
> - **Per-supplier margins after both fixes (Q1 2026, top by revenue):**
>   - შრომა-2023 +31.00% (was −26.28%) verified
>   - იბერია რეფრეშმენტსი +17.41% verified
>   - კოკა-კოლა გურია +17.67% verified
>   - იფქლი +14.70% verified
>   - ვასაძის პური +13.55% verified
>   - ELIZI (Camel/Sobranie/Winston) +7.78% protected
>   - ჯიდიაი (Parliament) +6.80% protected
>   - ზედაზენი აჭარა +2.11% verified (likely under-stated — see Open §6)
>   - Portfolio profit: +93,939 ₾ (was −611,380 ₾ this morning, +62,535 ₾ after the ELIZI fix)
> - **Tests**: 57/57 supplier_profitability + retail_sales_revenue_formula green at every step. Pipeline runs clean on C:\ — 0 errors / 0 warnings / data.json 52.34 MB.
