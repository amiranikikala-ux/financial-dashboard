...
> **განახლდა**: 2026-05-02 (post-midnight extension, browser-tested live) — Phase 2 LANDED + post-ship fix for HTTP 400. 2 commits ahead of last handoff: `e128669` feat (data layer + frontend) and `2e16485` fix (api_contracts registration that the feat commit forgot). User now actively cleaning empty/duplicate categories in MegaPlus via the live tab — corrected products will disappear from the table on next ZIP backup ingest. Roadmap → `docs/MASTER_PLAN.md`. წესები → `AGENTS.md`.
>
...
> 1. **🟢 GET table cross-check vs rs.ge waybills** — 49,790 Megaplus-internal purchase records. Compare against rs.ge ფაქტურა (already loaded). Surfaces: cancelled-on-rs-but-accepted-in-Megaplus (ghost AP — real money), accepted-on-rs-but-not-in-Megaplus (missing inventory). Light task, high financial value. **TOP PICK** for next session.
