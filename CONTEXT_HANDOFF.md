...
- **Tool surface**: **26 tools live** (Phase 2.9 added `detect_trends` @ 17, Sprint 5.1 added `get_vat_reconciliation_month` @ 18, `explain_unaccounted_cash` @ 19, `record_cash_outflow` @ 20). Index positions: `compute_cash_runway` @ 11, `compute_cash_flow_projection` @ 12, `build_debt_repayment_plan` @ 13, `simulate_scenario` @ 14, `analyze_product_profitability` @ 15, `find_promotion_candidates` @ 16, `detect_trends` @ 17, `get_vat_reconciliation_month` @ 18, `explain_unaccounted_cash` @ 19, `record_cash_outflow` @ 20, `propose_feature` @ 25 (tail). Tools emitting `summary_ka` verbatim (10 by description mandate + dead_stock/forecasting status fields): dog-foodable tail of the surface. Raw-data readers + CRUD confirms (read_data_json, compute, recall_context, save_memory, journal_*, read_source_code, grep_code, read_excel_source, validate_vs_source) **skip** summary_ka.
...
### Sprint 5.2 — TBC POS terminal-ID fix ✅ **LANDED 2026-04-23 (commit pending)**

`tbc_card_income_patterns.json` გადაიყვანა terminal-ID-based matching-ზე (5 ფიზიკური TBC ტერმინალი: RS014189, SH079927, SH046092, SH034467, SH060853). ძველი `match_substrings` (ნავაჭრი, ტერმინალებში მიღებული, ecom/pos, wallet/domestic) + `iban_hints` (GE69TB... transit IBAN) ორმაგად ითვლიდა end-of-day transit sweeps per-transaction rows-ის გვერდით.

**Live-verified cross-check (3 independent sources, 37 თვე 2023-01 → 2026-01):**
| წყარო | TBC POS | BOG POS | Total POS |
|---|---:|---:|---:|
| RS.ge POS terminal file (ground truth) | 384,534 ₾ | 1,571,581 ₾ | 1,956,115 ₾ |
| State audit (გაანგარიშება შპს ჯეო ფუდთაიმი.xlsx) | 364,230 ₾ | 1,271,593 ₾ | 1,635,823 ₾ |
| **Pipeline post-fix** | **378,735 ₾** | **1,481,043 ₾** | **1,859,778 ₾** |
| Pipeline pre-fix | 3,025,046 ₾ | 1,433,103 ₾ | 4,458,149 ₾ |

**Pipeline vs RS (ground truth)**: TBC 98.5%, BOG 94%, combined 95% match. Pipeline vs audit: TBC 104%, BOG 116% — audit systematically under-counts POS.

**Cumulative real vs declared (37 თვე, post-fix regen):**
| | ₾ |
|---|---:|
| declared (ბუღალტერი) | 3,903,150 |
| MAX retail revenue | 5,039,708 |
| bank_card (TBC+BOG) | 1,859,778 |
| cashreg_in (= MAX − bank) | 3,286,825 |
| invoices (ა/ფ bruto) | 491,039 |
| **total_real_ge** | **5,637,642** |
| **gap_vs_declared** | **+1,595,306** |

**Key insight**: TBC fix-ით gap არსებითად არ შეიცვალა (1,580K → 1,595K, +15K ზრდა). TBC overcount 2.80M ₾ იყო bank_card-ში, მაგრამ კომპენსირდებოდა cashreg_in floor-ის გამო (bank_exceeds_max case-ში cashreg=0). Post-fix ქვოტები **სანდო** და **cross-validated 3 წყაროთი**. **Audit-ის 742K "ხარვეზი" რეალურად 1.6M ₾**-ია — pipeline რეალურ შემოსავალს 1M ₾-ით მეტად იჭერს ვიდრე audit.

**cash_unaccounted 3.29M ₾ cumulative** (ყველა cashreg_in, ჯერ არცერთი კლასიფიცირებული). VAT liability preview (18%) = **591K ₾** თუ არავინ კლასიფიცირდება cash_outflow_journal.csv-ში. უდიდესი წილი legitimate გახარჯვაა (supplier ხელზე, ხელფასი ნაღდი, personal withdrawal) — user classification Sprint 5.3 scope.

**Data completeness caveats:**
- 2023-01 → 2023-05: pipeline MAX retail_sales ფაილი არ მაქვს, ამიტომ real<declared (negative gap -15K..-30K/თვე). **არ არის real under-declaration — MAX data gap**.
- 2023 Q1-Q4 TBC: residual +40-80K (pipeline-ში დამატებითი POS-თან მსგავსი rows რომელიც RS file-ში არ ფიქსირდება). <3% cumulative delta.

**Implementation (commit pending):**
- `dashboard_pipeline/bank_income.py` — `_DEFAULT_TBC_TERMINAL_IDS` + `_tbc_income_row_has_terminal` + `collect_tbc_card_income` terminal-filter logic
- `Financial_Analysis/tbc_card_income_patterns.json` — `terminal_ids` replaces `match_substrings` + `iban_hints`
- `tests/test_tbc_pos_terminal_matching.py` — 10 new tests (config shape, case-sensitivity, fallback, end-to-end)

---

### [ARCHIVED] Sprint 5.1 delivery details
