...
| მეტრიკა | მნიშვნელობა |
|---|---|
| **pytest** | **2,087/2,087 green** (~75s full run; +14 regression pins Sprint 3b/3c incremental cache) |
...
|---|---|---|---|---|
| **1** | **Tier 2 Sprint 3d — tax_flow cache (cross-bank)** | **~1 session** | **MED** | `collect_tax_flow` is the only cross-bank collector — iterates BOTH `list_tbc_bank_statement_xlsx()` AND `list_bog_bank_statement_xlsx()`, tags rows with `ბანკი: "TBC"|"BOG"`. Per-file cache is safe (file paths are disjoint by bank directory). Fingerprint must cover `tax_flow_patterns.json` + hardcoded `default_patterns` + `treasury_in_markers` list + `TAX_TREASURY_CLUSTER_NOTE_KA` constant. `_run_cached_per_file` helper already generic. |
| 2 | **Tier 2 Sprint 3e — POS terminal income cache (Sprint 5.12-sensitive)** | **~1 session** | **MED-HIGH** | `collect_tbc_card_income` + `collect_bog_pos_terminal_income`. Sprint 5.12 audit-defense depends on TBC card_income totals reproducing exactly. **Must add pin test** (`tests/test_tbc_card_income_cache_equivalence.py`) asserting pre-cache vs post-cache dict-identical output on the current `Financial_Analysis/თბს ბანკი ამონაწერი/` corpus BEFORE wiring use_cache=True. |
| 3 | **Tier 2 Sprint 3f — foodmart cashback** | **~1 session** | LOW | `collect_tbc_foodmart_cashback` — smallest collector, easy follow-up once 3d/3e land. |
...
- Sprint 3c (`0a81b86`) — expense_categories per-file cache landed, 62x TBC / 246x BOG speedup, 7 new tests (2,087/2,087 green); `_run_cached_samurneo` generalized to `_run_cached_per_file`
