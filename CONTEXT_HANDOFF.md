...
2. **Tier 2 Sprint 3 — extend cache pattern to bank / supplier / waybills** (~1 session each). The retail_sales refactor is the template: extract per-file processing → return serializable payload → merge in orchestrator. `pipeline_cache` module already supports multiple sections via normalized path keys. **Watch cache size** — 905 MB from retail_sales alone suggests per-file payloads are heavy; consider trimming payloads to just the merge-essential fields before Sprint 3 (drop preview_rows, minimize string duplication, etc.).
