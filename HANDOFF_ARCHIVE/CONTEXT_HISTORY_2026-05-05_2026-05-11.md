# CONTEXT HISTORY — 2026-05-05 → 2026-05-11

> Archived 2026-05-12 from CONTEXT_HANDOFF.md. Sessions in reverse chronological order.

## 0a. წინა session-ის შედეგი (2026-05-11 ღამე) — Cashiers/Salaro rebuild

### Headline

Owner showed a MegaPlus screenshot of „სალარო" (cash register) window and wanted the dashboard Cashiers tab to match it 1:1 — same layout, same numbers, same names. Previous Cashiers page was dark-themed Mercury-ish UI that owner rejected. After 3 visual iterations (light Windows look → kept dark theme but mirrored MegaPlus structure) + 2 data-shape fixes (per-shift → per-day; user_id only → user_id + name), the page now matches MegaPlus screenshots exactly.

### Visual changes — `rs-dashboard/src/Cashiers.jsx`

- Full rewrite as `.salaro-page` scoped component.
- Layout (matches MegaPlus „სალარო" window):
  - Title bar: blue gradient icon + „სალარო"
  - Toolbar: ▶ play · date-from input · 📅 calendar icon · 🕐 clock icon · date-to input · store dropdown · cashier dropdown · ▲ green clear-arrow
  - Cashier table: მოლარე / ნაღდი / უნაღდო / მიმდინარე / ჯამი / რაოდენობა
  - 6-box KPI strip: ნაღდი / უნაღდო / მიმდინარე / ჯამი (highlighted) / კლიენტები / საშუალო
- Date-picker: clicking any of (▶, date inputs, 📅, 🕐) opens a unified inline popup with `react-datepicker` (range mode) + per-side time selectors. Dark theme styled (`.sl-pop` scope).
- Store dropdown — when „დვაბზუ" selected, cashier dropdown's default option auto-updates to „ყველა მოლარე — დვაბზუ" + cashier list filters to that store's users. Cashier filter resets to „all" on store change.
- Default date — auto-set to last fully-loaded day from `cashier_day_breakdown` (`2026-05-09` after the May 9 backup was processed; was `2026-05-08` before).

### Backend changes — `dashboard_pipeline/megaplus_backup.py`

- **USERS table discovery (key finding)**: comment in old code said „STAFF table is empty so we can only show ORD_USER_ID" — that's true for STAFF, but **USERS table has cashier names** (column `username`). Both store DBs have it:
  - `MEGAPLUS_1329` (დვაბზუ) — 7 users: მენეჯერი, მოლარე1, ცირა კუტუბიძე, ქეთი დობორჯგინიძე, **დოდო დარჩია**, **ლია შკუბულიანი**, ნატო
  - `MEGAPLUS_1301` (ოზურგეთი) — 8 users: მენეჯერი, შორენა ხელაძე, ქრისტინე ხურციძე, სალომე ბურაკოვი, მოლარე 1, **ანდრია**, **ლიკა სვანი**, მედეა ზარმაცი
  - user_id namespace is **per-store** (user_id=6 in დვაბზუ = ლია; user_id=6 in ოზურგეთი = ანდრია — totally different people).
- `cashiers` SQL extended with `LEFT JOIN USERS u ON u.U_ID = o.ORD_USER_ID` + `MAX(u.username) AS cashier_name`.
- `shifts` SQL same JOIN + `cashier_name` column added.
- **New SQL `cashier_day_breakdown`** — per-DAY per-user (calendar-day window 00:00–23:59), so dashboard filter now matches MegaPlus's own „26/05/08 00:00–23:59" filter. Window from 2025-01-01. Per-row: day, user_id, cashier_name, lines, receipts, cash, card, revenue, aov. The cash/card split is real (CASE WHEN ORD_PAY_TYP=0/1) so per-cashier per-day ნაღდი/უნაღდო is exact, not approximated.
- Old `shifts` data still exposed (cross-midnight shifts e.g. 7 მაი 08:01 → 8 მაი 01:01 still bundled into one shift_id) — kept for backwards compatibility but UI now uses `cashier_day_breakdown` instead.

### Pipeline + API contracts

- `dashboard_pipeline/retail_sales.py` — combined accumulator `combined_cashier_day_breakdown` across stores, each row tagged with `object` (store label).
- `dashboard_pipeline/api_contracts.py` — `cashier_day_breakdown` added to retail_sales allowlist.

### Backup freshness procedure — important

The default refresh script `_scratch_refresh_megaplus_live.py` only **re-reads the already-restored DB** (uses state's `last_processed_zip`). To process a **newer ZIP**, run `process_newest_backup(folder, force=True)` directly. This session did that to pick up the May 9 ZIP (was stuck on May 8):

```powershell
"C:\Users\tengiz\OneDrive\Desktop\AI აგენტი\venv\Scripts\python.exe" -c "
from pathlib import Path
from dashboard_pipeline.megaplus_backup import process_newest_backup, _discover_watch_folders
for f in _discover_watch_folders(Path(r'C:\financial-dashboard\Financial_Analysis')):
    r = process_newest_backup(f, force=True)
    print(f.name, r and r.get('backup_date'))
"
```

After that: `_scratch_resynth_retail_sales.py` → `npm run build`.

### Verification — May 8 2026, both stores match MegaPlus exactly

| store | cashier | ჩვენი (cash / card / revenue / receipts) | MegaPlus (same) |
|---|---|---|---|
| დვაბზუ | ლია შკუბულიანი | 95.48 / 31.39 / 126.87 / 9 | 95.48 / 31.39 / 126.87 / 9 ✅ |
| დვაბზუ | დოდო დარჩია | 2173.10 / 1715.08 / 3888.18 / 390 | 2173.07 / 1715.07 / 3888.16 / 390 ✅ |
| ოზურგეთი | ანდრია | 497.80 / 298.13 / 795.93 / 95 | 497.81 / 298.13 / 795.94 / 95 ✅ |
| ოზურგეთი | ლიკა სვანი | 1673.43 / 912.05 / 2585.48 / 392 | 1673.44 / 912.05 / 2585.50 / 392 ✅ |

Owner-confirmed: „დვაბზუ შევამოწმე სწორია" + „ოზურგეთიც ემთხვევა" (rounding 0.01-0.03 ₾ accepted as expected).

### Files changed this session — **uncommitted on local main**

- `rs-dashboard/src/tabConfig.js` (label „მოლარეები" → „სალარო")
- `rs-dashboard/src/Cashiers.jsx` (full rewrite, ~480 lines)
- `dashboard_pipeline/megaplus_backup.py` (USERS JOIN + `cashier_day_breakdown` SQL + return-dict entry)
- `dashboard_pipeline/retail_sales.py` (combined accumulator + return-dict entry)
- `dashboard_pipeline/api_contracts.py` (allowlist)
- `Financial_Analysis/cashier_names.json` (**delete this** — created early in session as a manual fallback before USERS table was discovered; obsolete now)

Both earlier May 11 commits (`f9a78e2`, `d0b924f`) also still local main, not pushed.

### Pending — for next session

- Owner did NOT authorize commit/push yet (per pattern). 4 of the 5 May 11 changes (retail-sales period filter + Cashiers/Salaro rebuild) ready to commit.
- TBC bank refresh sidetrack — owner clicked refresh button at 20:31 local; got TBC ❌ skipped („auto-refresh does not consume OTP"). Likely cause: auto-scheduler ran concurrently and his manual OTP request got rate-limited or `already_running`. Not a code bug — just timing. Owner can retry with a fresh DigiPass code anytime.
- Delete obsolete `Financial_Analysis/cashier_names.json` (no longer needed — USERS table is canonical).

### Verification commands (next session)

```powershell
# Per-day cashier breakdown for May 8 (both stores) — should match MegaPlus
"C:\Users\tengiz\OneDrive\Desktop\AI აგენტი\venv\Scripts\python.exe" -c "
import urllib.request, json
api = json.loads(urllib.request.urlopen('http://localhost:8000/api/data?tab=retail_sales', timeout=30).read())
rs = api['retail_sales']
may8 = sorted([r for r in rs['cashier_day_breakdown'] if r['day']=='2026-05-08'], key=lambda x:(x['object'], -x['revenue']))
for r in may8:
    print(f\"{r['object']:<10} {r['cashier_name']:<20} cash={r['cash']:.2f} card={r['card']:.2f} rev={r['revenue']:.2f} rec={r['receipts']}\")
"
# expect 4 rows matching the table above
```

---

## 0a. წინა session-ის შედეგი (2026-05-11 დღე) — Retail Sales period-filter sprint
...
## 0a. წინა session-ის შედეგი (2026-05-10 → 2026-05-11) — Code Dispersion sprint
